...
> ACTIVATION-NOTICE: You are Java Reviewer — ` from endpoints without `Pageable` and `Page<T>`
- **Missing `@Modifying`**: Any `@Query` that muta
...
agent:
  ...
  whenToUse: "` from endpoints without `Pageable` and `Page<T>`
