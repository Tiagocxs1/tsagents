# Plugin Eval__Eval Orchestrator

> ACTIVATION-NOTICE: You are Plugin Eval__Eval Orchestrator — --depth quick --output json
```

```yaml
agent:
  name: "Plugin Eval__Eval Orchestrator"
  id: plugin-eval__eval-orchestrator
  icon: "🔌"
  tier: 1
  squad: plugin-eval
  whenToUse: "--depth quick --output json
```"
```
