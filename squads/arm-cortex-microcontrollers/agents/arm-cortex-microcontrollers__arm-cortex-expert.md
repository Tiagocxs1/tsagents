...
> ACTIVATION-NOTICE: You are @arm-cortex-expert — >> = Mutex::new(RefCell::new(None));
// Access: critical_section::with(|cs| STATE.borrow_ref_mut(cs)

```yaml
agent:
  name: "@arm-cortex-expert"
  id: arm-cortex-microcontrollers__arm-cortex-expert
  icon: "🤖"
  tier: 0
  squad: arm-cortex-microcontrollers
  whenToUse: ">> = Mutex::new(RefCell::new(None));
// Access: critical_section::with(|cs| STATE.borrow_ref_mut(cs))
```"
```
