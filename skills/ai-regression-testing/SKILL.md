...
Fix 4: Test caught it instantly on first run Ã¢Å“â€¦
```
...
```typescript
// Ã¢ÂÅ’ AI adds field to production path only
if (isSandboxMode()) {
...
// Ã¢Å“â€¦ Both paths must return the same shape
if (isSandboxMode()) {
...
```typescript
// Ã¢ÂÅ’ New column added to response but not to SELECT
...
// Ã¢Å“â€¦ Use SELECT * or explicitly include new columns
const { data } = await supabase
  .from("users")
  .select("*")
  .single();
...
```typescript
// Ã¢ÂÅ’ Error state set but old data not cleared
catch (err) {
  setError("Failed to load");
  // reservations still shows data from previous tab!
}

// Ã¢Å“â€¦ Clear related state on error
catch (err) {
...
// Ã¢Å“â€¦ Capture previous state and rollback on failure
const handleRemove = async (id: string) => {
...
|---|---|---|
| Sandbox/production mismatch | Assert same response shape in sandbox mode | Ã°Å¸â€Â´ High |
| SELECT clause omission | Assert all required fields in response | Ã°Å¸â€Â´ High |
| Error state leakage | Assert state cleanup on error | Ã°Å¸Å¸Â¡ Medium |
| Missing rollback | Assert state restored on API failure | Ã°Å¸Å¸Â¡ Medium |
| Type cast masking null | Assert field is not undefined | Ã°Å¸Å¸Â¡ Medium |
