---
name: configure-claude-fulcrum
description: Interactive installer for Claude Fulcrum Ã¢â‚¬â€ guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
origin: claude-fulcrum
---

# Configure Claude Fulcrum (Claude Fulcrum)

An interactive, step-by-step installation wizard for the Claude Fulcrum project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure ecc", "install ecc", "setup Claude Fulcrum", or similar
...
1. **Via Plugin**: `/plugin install claude-fulcrum` Ã¢â‚¬â€ the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.claude/skills/configure-claude-fulcrum/SKILL.md`, then activate by saying "configure ecc"
...
```bash
rm -rf /tmp/claude-fulcrum
git clone https://github.com/ORION2809/claude-Fulcrum.git /tmp/claude-fulcrum
```

Set `ECC_ROOT=/tmp/claude-fulcrum` as the source for all subsequent copy operations.
