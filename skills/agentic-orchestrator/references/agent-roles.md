...
- Before Builder starts, run a **reuse scan**: grep/glob for similar functions, components, utilities in the codebase
- Report existing implementations that overlap with planned work — include file paths and function signatures
...
- Reference existing components found by Reader's reuse scan — reuse or extend them rather than creating duplicates
- If creating new code despite existing similar implementations, state explicit justification in the execution summary
...
- Check for introduced duplication: new code that reimplements functionality already available in the codebase is a gate failure
