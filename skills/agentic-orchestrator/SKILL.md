...
### 9. Clean & Simple Code
Write the simplest implementation that satisfies the requirements. No speculative abstractions — only abstract when 3+ concrete uses exist. No unnecessary generics unless used by 2+ callers. Prefer composition over inheritance. Functions stay under ~40 lines (split if longer). Implement exactly what was asked, nothing more. If it works and it's readable, it's done.
...
1. **Reader** (if context needed) -- Gather specific files, functions, symbols. Run reuse scan for similar existing implementations. Produce concise context pack including reuse candidates.
...
**Periodic evaluation checkpoint (every 4-5 completed tasks):**
Spawn a fresh QA subagent (independent perspective, clean context) to evaluate cumulative work:
1. **Code quality:** Run lint, build, full test suite. Scan modified files for DRY violations.
2. **Plan alignment:** Compare delivered work against original task definitions — any drift or missed requirements?
3. **Architectural consistency:** Do completed tasks form a coherent whole? Conflicting patterns?
4. **Best practices:** Function sizes, naming, error handling, test coverage adequacy.

The evaluator grounds every finding in evidence (tool output, line references). Gate failures create repair subtasks. Advisory findings get noted for future tasks. See `references/output-templates.md` Template H for the structured output format.
...
For structured output templates (Planning, Execution, QA, Context Guidance, Phase Summary, Evaluation Checkpoint), read `references/output-templates.md`.

## Harness Configuration

For accuracy-optimized settings (reasoning effort, hooks, context discipline), read `references/harness-config.md`.
