...
This skill ships with Claude Fulcrum. No separate installation is needed when Claude Fulcrum is installed.

### Full ECC install

If you are working from the Claude Fulcrum repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the Claude Fulcrum diff before updating:

```bash
cd /path/to/claude-fulcrum
...
If you are vendoring only this skill outside the full ECC install, copy the reviewed file from the Claude Fulcrum repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed ECC commit rather than running `git pull`.
