...
1. **Load Composition Framework** — carregue `composition-framework` skill e defina:
   - Layer stack com mínimo 6 camadas (atmosfera → grid → 3D → fotografia → glass card → texto)
   - 5+ técnicas simultâneas (gradiente atmosférico, gradient text, glassmorphism, multi-shadow, etc.)
   - Asset Check: 3D FluentUI? Foto Unsplash? Ícone Phosphor?

2. **Create HTML/CSS banner** using `composition-framework` + `social-posts-html` skills
   - Use exact platform dimensions from size reference
   - Apply safe zone rules (critical content in central 70-80%)
   - Max 2 typefaces, single CTA, 4.5:1 contrast ratio
   - Inject brand context via `inject-brand-context.cjs`

3. **Generate visual elements** with `ai-artist` + `ai-multimodal` skills
...
4. **Compose final banner** — overlay text, CTA, logo on generated visual in HTML/CSS, usando técnicas do composition-framework (glass cards, gradient text, multi-shadow, badge premium, CTA gradient)
