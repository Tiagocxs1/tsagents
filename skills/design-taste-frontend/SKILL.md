...
## Appendix C — Social Media Carrossel Design Rules (Instagram 1080×1350)

### C.1 Canvas & Grid
- **Canvas:** 1080×1350 (4:5 Instagram ratio)
- **Padding lateral:** 75px fixos (conteúdo = 930px útil)
- **Padding vertical:** 45px topo, 50px base
- **Linhas-guia verticais:** sutis (opacidade 0.03-0.04) a 75px das bordas

### C.2 Font Size Floor (mandatory for readability)
No carrossel 1080×1350, estes são os tamanhos MÍNIMOS:

| Elemento | Tamanho | Peso |
|---|---|---|
| Headline principal | 60-82px | 900 |
| Subheadline (body) | 22px | 400 |
| Eyebrow / label de motivo | 15px | 700 |
| Tags / chips | 17px | 600 |
| CTA button | 18-20px | 700-800 |
| Estatísticas (números) | 28-34px | 900 |
| Labels de métrica | 14px | 600 |
| Texto auxiliar (small print) | 14px | 400 |

**Regra de ouro:** se o texto parece pequeno no seu preview mental, aumente. 22px para corpo não é grande — é o mínimo legível em 1080px de largura.

### C.3 The Subheadline Width Rule (CRITICAL)
**O texto do corpo NUNCA pode ultrapassar a largura da headline acima dele.** Esta é a regra de design mais violada por IAs.

- Meça a largura aproximada da headline (chars × font-size × 0.55 + letter-spacing)
- Aplique `max-width` no `<p>` igual ou menor que esse valor
- Em layouts split (texto + imagem): `max-width: 420px` para corpo com headlines de 60-64px
- Em layouts full-width: `max-width: 520-580px`
- Em layouts centralizados: `max-width: 480px`

### C.4 Layout Diversification Per Slide
Em um carrossel de 7 slides, cada slide DEVE ter uma estrutura única:

1. **Full-bleed hero** — imagem ocupa 55% do topo, headline sobreposto, stats no rodapé
2. **Split 55/40** — texto à esquerda (55%), imagem retrato à direita (40%)
3. **Stack vertical** — imagem faixa no topo (380px), texto abaixo
4. **Split 40/55** — imagem retrato à esquerda (40%), texto à direita (55%) — espelho do slide 2
5. **Texto + imagem flutuante** — imagem pequena (240px) alinhada ao centro vertical à direita
6. **Background full com overlay** — imagem cobre 100%, overlay escuro gradiente, texto centralizado
7. **Background lifestyle + CTA** — imagem cobre 100%, CTA centralizado + métricas no rodapé

**Proibido:** repetir o mesmo layout em 2 slides consecutivos. Cada slide deve ter uma composição visual distinta.

### C.5 Image Orientation Variety
Não use todas as imagens na mesma orientação. Alterne:

| Slide | Orientação | Uso |
|---|---|---|
| 01 | Paisagem (hero) | 55% do topo, full-width |
| 02 | Retrato (vertical) | Coluna de 40% à direita |
| 03 | Paisagem | Faixa no topo (380px altura) |
| 04 | Retrato (vertical) | Coluna de 40% à esquerda |
| 05 | Retrato (phone) | Quadro pequeno 240px à direita |
| 06 | Paisagem (abstract) | Background full |
| 07 | Paisagem (lifestyle) | Background full |

### C.6 Ratios & Proporções
- **Split com imagem:** imagem = 40% da largura, texto = 55% (gap 5% = 45px)
- **Imagem retrato:** `w=500` no Unsplash, `object-fit: cover` no CSS
- **Imagem paisagem faixa:** `w=1080`, altura fixa 380px
- **Imagem flutuante (phone):** 240px largura, `w=400` no Unsplash
- **Background full:** `w=1080`, `object-fit: cover`

### C.7 Rendering via Pictify (HTML-to-Image)
Ao gerar imagens via Pictify API:
- **Sempre usar inline styles** (sem `<style>` tags externas, sem classes CSS)
- **NUNCA depender de @import** para fontes externas — use `font-family: sans-serif` (fallback do sistema)
- **Imagens:** URLs absolutas de Unsplash com parâmetros `?q=80&w=N&auto=format&fit=crop`
- **Canvas:** passar `width=1080, height=1350` no body da API
- **Evitar** `backdrop-filter`, `filter: blur()` excessivo (Pictify pode não renderizar)
- **Verificar** se todas as imagens carregaram comparando tamanho do arquivo: >80KB = OK, <40KB = imagens quebradas

### C.8 Hierarchy Lock
1. Eyebrow/label (15px, uppercase tracking 3px)
2. Headline (60-82px, weight 900, letter-spacing -2.5 a -3px)
3. Subheadline/body (22px, line-height 1.5, max-width constraint)
4. Tags (17px, rounded 10px, subtle border)
5. CTA button (18-20px, gradient bg, shadow)

Nenhum elemento visual deve competir com a headline. O CTA é o segundo elemento mais importante — deve ter contraste máximo.
## Appendix D — WCAG Contrast Rules for Carrossel & Visual Design

### D.1 WCAG Contrast Ratios (Referência)

| Nível | Texto Normal (<24px regular ou <19px bold) | Texto Grande (≥24px regular ou ≥19px bold) |
|---|---|---|
| **AA (mínimo)** | **4.5:1** | **3:1** |
| **AAA (reforçado)** | **7:1** | **4.5:1** |

### D.2 Classificação dos Elementos do Carrossel

| Elemento | Tamanho | Classificação | Contraste Mínimo |
|---|---|---|---|
| Headline | 60-82px | Grande | 3:1 (AA) / 4.5:1 (AAA - alvo) |
| Subheadline/body | 22px | Grande | 3:1 (AA) / 4.5:1 (AAA - alvo) |
| Eyebrow/label | 15px | Normal | 4.5:1 (AA) / 7:1 (AAA) |
| Tags/chips | 17px | Normal | 4.5:1 (AA) |
| CTA button text | 18-20px | Grande (bold) | 3:1 (AA) / 4.5:1 (AAA) |
| Small print | 14px | Normal | 4.5:1 (AA) |
| Dots/pagination (slide 1) | 9px circulo | Graphical | 3:1 (AA, WCAG 2.2) |

### D.3 Regra de Contraste para Overlays em Imagens

Quando um slide usa **background image com overlay**, seguir estas regras:

1. **Overlay duplo obrigatório:** uma camada de fundo **sólido escuro** + uma camada **gradiente** (nunca apenas gradiente)
2. **Opacidade mínima do overlay sólido:** 0.78 para imagens escuras, 0.85+ para imagens claras, 0.92+ para imagens muito claras (céu, paisagens ensolaradas)
3. **Regra prática:** olhe para a imagem de fundo. Se for predominantemente escura (noite, interior, urbano escuro), opacidade 0.78-0.82 basta. Se for clara (dia, céu, praia), precisa 0.88-0.95.
4. **Overlay gradiente:** sobrepor o sólido com `linear-gradient(180deg, rgba(0,13,18,0.7), rgba(0,10,14,0.95))` — o gradiente resolve áreas onde o sólido sozinho não cobre variações de brilho.
5. **Cores de overlay recomendadas:**
   - Fundo sólido: `#000d12` ou `#000a0e` ou `#021018` (mais escuro que `#022636`)
   - Overlay duplo padrão: sólido `opacity: 0.82` + gradiente `rgba(0,0,0,0.2)→rgba(0,10,14,0.95)`
6. **Teste visual:** em qualquer ponto do slide, a área onde há texto deve ter contraste ≥ 4.5:1 com o texto
7. **Exceção:** se o texto for apenas headline branca sobre fundo escuro sólido (sem imagem), usar fundo `#022636` já oferece ~17:1 de contraste com branco — OK sem overlay
8. **Exceção para imagens claras (ex: fachada ensolarada, slide 7 lifestyle):** opacidade do overlay sólido precisa ser ≥ 0.88 e a cor sólida precisa ser `#000a0e` (mais escura ainda). Em casos extremos (imagem com áreas quase brancas), usar `opacity: 0.95` no overlay sólido.

### D.4 Como Calcular a Opacidade do Overlay

Para garantir contraste de texto branco (#fff) sobre uma imagem qualquer:

1. Estime a **luminância média da área do texto** na imagem original (0 = preto, 1 = branco). Uma foto típica tem L ≈ 0.18-0.35; uma foto clara (lifestyle, céu) tem L ≈ 0.5-0.65.
2. Após overlay de opacidade `o` e cor `#000d12` (L ≈ 0.013):
   ```
   L_result = L_imagem * (1 - o) + 0.013 * o
   ```
3. Contraste do texto `#fff` (L = 1.0) sobre o resultado:
   ```
   ratio = (1.0 + 0.05) / (L_result + 0.05)
   ```
4. Para AA (4.5:1), L_result precisa ser ≤ 0.183
5. **Tabela prática (texto branco sobre overlay #000d12):**

| L_imagem | overlay 0.75 | overlay 0.82 | overlay 0.88 | overlay 0.92 | overlay 0.95 |
|---|---|---|---|---|---|
| 0.2 (escura) | 3.5:1 ⚠️ | 4.6:1 ✓ AA | 5.9:1 ✓ | 7.4:1 ✓ | 9.6:1 ✓ |
| 0.4 (média) | 2.8:1 ❌ | 3.6:1 ⚠️ | 4.6:1 ✓ AA | 5.7:1 ✓ | 7.3:1 ✓ |
| 0.6 (clara) | 2.4:1 ❌ | 3.0:1 ⚠️ | 3.8:1 ⚠️ | 4.6:1 ✓ AA | 5.8:1 ✓ |

**Conclusão:** overlay 0.82 funciona para imagens escuras, mas para imagens claras (típicas de lifestyle, slides 6-7) precisa de **0.88 a 0.95**. Sempre considerar o pior caso (área mais clara da imagem sob o texto).

### D.5 Armadilhas Comuns de Contraste

1. **Glassmorphism sobre imagem:** `background: rgba(X,Y,Z,0.6)` + `backdrop-filter: blur()` não garante contraste. Sempre teste sobre a imagem real.
2. **Text-gradient sobre overlay:** gradiente no texto (`#00e5ff` a `#00b4d8`) reduz contraste dramático. Preferir branco sólido para headlines com overlay. Se usar gradiente, aumentar overlay em 0.05-0.08.
3. **Opacidade em body text:** `rgba(255,255,255,0.5)` é só 2.4:1 sobre fundo médio. Mínimo seguro: `rgba(255,255,255,0.75)` para body, `rgba(255,255,255,0.55)` para small print.
4. **Overlay único gradiente:** um gradiente que vai de 0% a 100% sempre tem uma área clara no meio. Usar fundo sólido + gradiente sobreposto elimina esse problema.
5. **Cores de destaque em texto pequeno:** `#00ffff` sobre `#022636` tem 6.2:1 ✓ para grande, mas `#cb3033` sobre `#022636` tem só 3.8:1 ⚠️ — usar vermelho apenas para texto ≥24px ou adicionar bg escuro extra.
6. **Ignorar contraste do botão CTA:** botão com texto branco sobre bg claro pode falhar. Botão `#cb3033` com texto `#fff` tem 4.1:1 — tecnicamente AA para 18px bold (grande), mas só se o fundo atrás do botão for escuro. Se o botão flutua sobre imagem, o overlay precisa cobrir a área do botão também.

### D.6 Cores da Marca Guardian & Contraste

| Cor | Luminância | Ratio sobre `#000d12` | Ratio sobre `#fff` | Uso seguro |
|---|---|---|---|---|
| `#FFFFFF` (branco) | 1.000 | **19.8:1 ✓** | 1:1 | Headline, body, CTA |
| `#00FFFF` (ciano) | 0.787 | **15.6:1 ✓** | 1.3:1 ❌ | Headline destaque, badges (nunca sobre branco) |
| `#48CAE4` (ciano claro) | 0.480 | **9.6:1 ✓** | 2.2:1 ❌ | Apenas sobre fundo escuro |
| `#CB3033` (vermelho) | 0.131 | **2.9:1 ⚠️** | 6.3:1 ✓ | Sobre fundo escuro só como accent gráfico; usar sobre `#fff` ou dentro de card | 
| `#022636` (azul escuro) | 0.032 | **1.5:1 ❌** | **17.6:1 ✓** | Fundo de slide, não use como cor de texto |
| `#04344C` (azul médio) | 0.040 | **1.7:1 ❌** | **14.2:1 ✓** | Fundo de slide, não use como cor de texto |
| `#506A76` (cinza) | 0.084 | **3.2:1 ⚠️** | **8.8:1 ✓** | Fundo de slide; texto só se invertido (branco sobre ele) |

**Regras práticas para a paleta Guardian:**
- Texto **branco** sobre qualquer fundo da paleta: sempre OK
- **Ciano** (`#00FFFF`, `#48CAE4`): seguro sobre `#022636` e `#04344C` e `#000d12`, NUNCA sobre fundo claro
- **Vermelho** (`#CB3033`): perigoso sobre fundo escuro (3.8:1 sobre `#022636`, 2.9:1 sobre `#000d12`). Use apenas como:
  - Background de badge/botão com texto branco DENTRO
  - Texto grande ≥24px bold sobre fundo específico com contraste verificado
  - Elemento decorativo (linha, ícone) onde contraste não se aplica
- **Gradiente ciano** (`linear-gradient(135deg, #00ffff, #48cae4)`) sobre `#000d12`: a parte mais clara (#00ffff) tem 15.6:1, a mais escura (#48cae4) tem 9.6:1 — ambos OK

### D.7 Overlay por Tipo de Imagem (Casos Específicos)

| Tipo de Imagem Unsplash | Luminância Típica | Overlay Mínimo (sobre #000d12) | Notas |
|---|---|---|---|
| Fachada noturna (escura) | ~0.12-0.20 | 0.75-0.80 | Slide 4, 6 |
| Robô/tech azul escuro | ~0.15-0.25 | 0.78-0.82 | Slide 3 |
| Segurança/câmera (noturna) | ~0.10-0.18 | 0.75-0.78 | Slide 2 |
| Phone/tela (interior) | ~0.20-0.40 | 0.82-0.85 | Slide 5 |
| Lifestyle/casa clara | ~0.35-0.50 | 0.85-0.90 | Slide 7 |
| Lifestyle/ensolarado | ~0.45-0.65 | 0.88-0.95 | Slide 7 (caso extremo) |
| Nuvem/servidor (azul escuro) | ~0.10-0.20 | 0.78-0.82 | Slide 6 |
| Paisagem diurna | ~0.40-0.70 | 0.88-0.95 | Evitar para slides com texto longo |

### D.8 Como Testar Contraste Sem Ferramentas

Para verificar rapidamente se um overlay é suficiente:

1. **Teste do polegar:** coloque o polegar sobre a imagem original (simulando um overlay preto a ~80%). O texto branco deve ser confortavelmente legível. Se precisar apertar os olhos, aumente o overlay.
2. **Regra dos 3 tons:** o overlay escuro deve tornar a imagem original pelo menos 3 tons mais escura que o tom médio original. Se a imagem original tem tom médio 5/10, o overlay deve empurrá-la para 1-2/10.
3. **Teste do cinza:** converta mentalmente a área do texto para escala de cinza. Texto branco deve se destacar visivelmente do cinza resultante. Se parecer cinza-claro-sobre-cinza-médio, aumente o overlay.
4. **Regra prática:** para qualquer slide com imagem, NUNCA usar overlay < 0.78. O mínimo seguro para qualquer imagem é 0.82. Para imagens claras, 0.88-0.95.

### D.9 Fórmula Rápida de Contraste

```
contrast_ratio = (L1 + 0.05) / (L2 + 0.05)
```

Onde L é a luminância relativa (0 = preto, 1 = branco).
Luminância de uma cor sRGB: `L = 0.2126*R + 0.7152*G + 0.0722*B` (com valores lineares, após degamma).

**De-gamma simplificado:** para cada canal R/G/B em 0-255:
```
s = valor / 255
...
**Referências rápidas:**
- `#fff` sobre `#022636` = ~17.6:1 ✓ (AAA)
- `#fff` sobre `#01141c` = ~19.8:1 ✓ (AAA)
- `#fff` sobre `#000d12` com overlay 0.82 = ~5.3:1 ✓ (AA em imagem média)
- `rgba(255,255,255,0.75)` sobre `#000d12` = ~5.3:1 ✓ (AA)
- `rgba(255,255,255,0.5)` sobre `#000d12` = ~3.1:1 ⚠️ (só para texto grande 24px+)
- `#00ffff` sobre `#022636` = ~6.2:1 ✓ (AA)
- `#cb3033` sobre `#022636` = ~3.8:1 ⚠️ (só para texto grande, falha para normal)
- `#cb3033` sobre `#fff` = ~6.3:1 ✓ (AA)
- `#48CAE4` sobre `#022636` = ~5.8:1 ✓ (AA)
- `#ffff` sobre overlay 0.88 de `#000d12` em imagem clara = ~4.6:1 ✓ AA

### D.10 Checklist de Contraste (Pré-Envio)

**Essencial:**
- [ ] Headline tem no mínimo 3:1 contra o fundo/overlay (4.5:1 ideal)
- [ ] Body text (22px) tem no mínimo 3:1 (4.5:1 ideal)
- [ ] Labels/eyebrow (15px) têm no mínimo 4.5:1 (AA on normal text)
- [ ] Tags/chips (17px) têm no mínimo 4.5:1
- [ ] CTA/texto de botão tem no mínimo 3:1 (4.5:1 ideal)
- [ ] Small print (≤16px) tem no mínimo 4.5:1
- [ ] Overlay em slides com imagem é **duplo** (sólido + gradiente)
- [ ] Overlay considera a **área mais clara** da imagem sob o texto, não a média geral
- [ ] Overlay ≥ 0.82 para imagens escuras, ≥ 0.88 para imagens claras, ≥ 0.92 para muito claras
- [ ] Nenhum texto usa cor que falha contraste (ver D.6 para paleta Guardian)

**Avançado:**
- [ ] Cores de destaque (`#00ffff`, `#cb3033`) testadas individualmente contra o fundo real
- [ ] Botão CTA testado contra fundo + overlay (não apenas contra cor sólida)
- [ ] Text-gradient testado — se falhar, substituir por branco sólido
- [ ] Overlay não apaga detalhes da imagem (deve sobrar silhuetas reconhecíveis)
- [ ] Se houver glassmorphism, testado contra a imagem real, não contra fundo sólido
- [ ] Teste visual: imagem em miniatura (thumbnail) ainda é reconhecível após overlay
